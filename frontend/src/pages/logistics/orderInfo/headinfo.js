...
const HeadInfo = (props) => {
  ...
  const totalAmount = orderItems.length > 0 && orderItems?.reduce((sum, item) => {
      ...
      <Grid container spacing={1}>
        ...
        <Grid item xs={8} spacing={1}>
          <Grid item xs={12} sx={{ display: 'flex', direction: 'row', justifyContent: 'space-between' }}>
            ...
            <Typography variant="body1" sx={{ color: '#3498db' }}>{customerInfo.fullName}</Typography>
          </Grid>
          <Grid item xs={12} sx={{ display: 'flex', direction: 'row', justifyContent: 'space-between' }}>
            <Typography variant="body1">Email Address</Typography>
            <Typography variant="body1" sx={{ color: '#3498db' }}>{customerInfo.emailAddress}</Typography>
          </Grid>
          <Grid item xs={12} sx={{ display: 'flex', direction: 'row', justifyContent: 'space-between' }}>
            <Typography variant="body1">Contact Number</Typography>
            <Typography variant="body1" sx={{ color: '#3498db' }}>{customerInfo.account?.username}</Typography>
          </Grid>
          <Grid item xs={12} sx={{ display: 'flex', direction: 'row', justifyContent: 'space-between' }}>
            <Typography variant="body1">Address</Typography>
            <Typography variant="body1" sx={{ color: '#3498db' }}>{customerInfo.address}</Typography>
          </Grid>
          <Grid item xs={12} sx={{ display: 'flex', direction: 'row', justifyContent: 'space-between' }}>
            <Typography variant="body1">Payment Method</Typography>
            <Typography variant="body1" sx={{ color: '#3498db' }}>{orderInfo.paymentMethod}</Typography>
          </Grid>
        </Grid>
        <Grid item xs={12}>
          <MainCard sx={{ mt: 2 }}>
            <Grid container>
              ...
              <Grid item xs={12} sx={{ display: 'flex', direction: 'row', justifyContent: 'space-between' }}>
                ...
                <Typography variant="h4" sx={{ color: '#c0392b' }}>₱{totalAmount}</Typography>
              </Grid>
            </Grid>
          </MainCard>
        </Grid>

  ...
  useEffect(() => {
    if (data) {
      ...
      data['data'].map((s, i) => {
        ...
        newSched.push({ sched: s['date'], id: s['id'] });
        newSpap.push({ spap: s['description'], id: s['id'] });
      ...
      <Grid container spacing={1.5}>
        ...
        <Grid item xs={12} lg={3} direction="column">
          ...
          {listSchedule.map((item, index) => {
            return <Data field='date' title={item.sched} rowId={item.id} key={index} color='red' background={(index % 2 === 0) ? '#f2f2f2' : 'white'} />
          })}
        </Grid>
        <Grid item xs={12} lg={3} direction="column">
          <Header title="SP/AP" img="./asset/03.png" />
          {listspap.map((item, index) => {
            return <Data field='description' title={item.spap} rowId={item.id} key={index} background={(index % 2 === 0) ? '#f2f2f2' : 'white'} />
          })}
        </Grid>
        <Grid item xs={12} lg={3} direction="column">
          <Header title="TRUCK" img="./asset/truck.png" />
          {listtruck.map((item, index) => {
            return <Data field='plateNumber' title={item.plate} rowId={item.id} key={index} background={(index % 2 === 0) ? '#f2f2f2' : 'white'} />
          })}
        </Grid>
      </Grid >
